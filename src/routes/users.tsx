    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {users.length} utilisateur{users.length > 1 ? "s" : ""} · Gérez les accès à ThermoTrack
        </p>
      </div>
        ...
        {!showAdd ? (
          ...
          <div className="rounded-xl border border-primary/20 bg-card p-5 space-y-4">
              ...
              <div className="flex gap-3">
                {(["technicien", "admin"] as UserRole[]).map((r) => (
                  <label key={r} className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 cursor-pointer transition-all ${
                    ...
                    <span className="text-sm font-medium capitalize">{r === "admin" ? "Administrateur" : "Technicien"}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              ...
              <Button onClick={handleAdd} className="gap-2 glow-primary">
                  ...
                  <>
                    ...
                    <td className="px-4 py-2.5">
                      <div className="flex justify-end gap-1.5">
                        ...
                        <Button size="sm" className="h-8 gap-1.5 text-xs glow-primary px-3" onClick={handleSaveEdit}>
      ...
      <p className="mt-4 text-xs text-muted-foreground">
        Les mots de passe sont stockés localement dans le navigateur.
        L'administrateur peut réinitialiser n'importe quel mot de passe via le bouton Modifier.
      </p>
        ...
        <AlertDialogContent className="border-border bg-card">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-center">
              {toDelete && (
                <>L'utilisateur <span className="font-bold text-foreground">{toDelete.username}</span> sera supprimé définitivement. Cette action est <span className="text-destructive font-medium">irréversible</span>.</>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
